...
class CalcAgi
{
    ...
    public function sendCall($agi, $destination, &$MAGNUS, $typecall = 0)
    {
        ...
        if ($this->tariffObj[0]['FRCalled'] == 1) {
            ...
            if ($this->FRCalled == 1) {
                $sql = "SELECT id FROM pkg_restrict_called_range WHERE  id_user = $MAGNUS->id_user AND (number = SUBSTRING('" . $MAGNUS->destination . "',1,length(number))  OR number = SUBSTRING('" . $MAGNUS->CallerID . "',1,length(number))  ) ORDER BY LENGTH(number) DESC";
            } else {
        ...
        if ($this->tariffObj[0]['FRCaller'] == 1) {
            ...
            if ($this->FRCaller == 1) {
                $sql = "SELECT id FROM pkg_restrict_caller_range WHERE  id_user = $MAGNUS->id_user AND (number = SUBSTRING('" . $MAGNUS->destination . "',1,length(number))  OR number = SUBSTRING('" . $MAGNUS->CallerID . "',1,length(number))  ) ORDER BY LENGTH(number) DESC";
            } else {
                $sql = "SELECT id FROM pkg_restrict_caller_range WHERE  id_user = $MAGNUS->id_user AND number = SUBSTRING('" . $destination . "',1,length(number)) ORDER BY LENGTH(number) DESC";
            }
        ...
        if ($this->tariffObj[0]['FNCalled'] == 1) {
            ...
            if ($this->FNCalled == 1) {
                $sql = "SELECT id FROM pkg_restrict_called_number WHERE  id_user = $MAGNUS->id_user AND (number = SUBSTRING('" . $MAGNUS->destination . "',1,length(number))  OR number = SUBSTRING('" . $MAGNUS->CallerID . "',1,length(number))  ) ORDER BY LENGTH(number) DESC";
            } else {
                $sql = "SELECT id FROM pkg_restrict_called_number WHERE  id_user = $MAGNUS->id_user AND number = SUBSTRING('" . $destination . "',1,length(number)) ORDER BY LENGTH(number) DESC";
            }
        ...
        if ($this->tariffObj[0]['FNCaller'] == 1) {
            ...
            if ($this->FNCaller == 1) {
                $sql = "SELECT id FROM pkg_restrict_caller_number WHERE  id_user = $MAGNUS->id_user AND (number = SUBSTRING('" . $MAGNUS->destination . "',1,length(number))  OR number = SUBSTRING('" . $MAGNUS->CallerID . "',1,length(number))  ) ORDER BY LENGTH(number) DESC";
            } else {
                $sql = "SELECT id FROM pkg_restrict_caller_number WHERE  id_user = $MAGNUS->id_user AND number = SUBSTRING('" . $destination . "',1,length(number)) ORDER BY LENGTH(number) DESC";
            }
        ...
        if ($this->tariffObj[0]['id_trunk_history'] == 1) {
            $sql = "SELECT * FROM (SELECT pkg_cdr.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr.starttime FROM pkg_cdr INNER JOIN pkg_trunk_group_trunk ON pkg_cdr.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group'] . " UNION ALL SELECT pkg_cdr_failed.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr_failed.starttime FROM pkg_cdr_failed INNER JOIN pkg_trunk_group_trunk ON pkg_cdr_failed.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr_failed.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr_failed.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group'] . " AND pkg_cdr_failed.terminatecauseid != '5' 
                         UNION ALL SELECT pkg_cdr.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr.starttime FROM pkg_cdr INNER JOIN pkg_trunk_group_trunk ON pkg_cdr.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group2'] . " UNION ALL SELECT pkg_cdr_failed.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr_failed.starttime FROM pkg_cdr_failed INNER JOIN pkg_trunk_group_trunk ON pkg_cdr_failed.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr_failed.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr_failed.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group2'] . " AND pkg_cdr_failed.terminatecauseid != '5' 
                         UNION ALL SELECT pkg_cdr.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr.starttime FROM pkg_cdr INNER JOIN pkg_trunk_group_trunk ON pkg_cdr.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group3'] . " UNION ALL SELECT pkg_cdr_failed.id_trunk, pkg_trunk_group_trunk.id_trunk_group, pkg_cdr_failed.starttime FROM pkg_cdr_failed INNER JOIN pkg_trunk_group_trunk ON pkg_cdr_failed.id_trunk = pkg_trunk_group_trunk.id_trunk WHERE pkg_cdr_failed.starttime BETWEEN date_add(NOW(), INTERVAL -12 HOUR) AND NOW() AND pkg_cdr_failed.calledstation = '{$destination}' AND pkg_trunk_group_trunk.id_trunk_group = " . $this->tariffObj[0]['id_trunk_group3'] . " AND pkg_cdr_failed.terminatecauseid != '5' ORDER BY starttime DESC) AS TT Group by id_trunk ORDER BY starttime DESC LIMIT " . $this->tariffObj[0]['History_Number'] . " ";
        }
        ...
        if ($this->tariffObj[0]['trunk_group_type'] == 1) {
            $sql1 = "SELECT * FROM ( SELECT C.id, C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY CO.id ASC";
            $sql2 = "SELECT * FROM ( SELECT C.id, C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group2'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY CO.id ASC";
            $sql3 = "SELECT * FROM ( SELECT C.id, C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group3'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY CO.id ASC";
        } else if ($this->tariffObj[0]['trunk_group_type'] == 2) {
            $sql1 = "SELECT * FROM ( SELECT C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY starttime ASC";
            $sql2 = "SELECT * FROM ( SELECT C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group2'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY starttime ASC";
            $sql3 = "SELECT * FROM ( SELECT C.id_trunk, O.trunkcode, id_trunk_group, starttime FROM pkg_trunk_group_trunk C INNER JOIN pkg_trunk O ON C.id_trunk = O.id WHERE starttime <= NOW() - INTERVAL 10 SECOND AND id_trunk_group = " . $this->tariffObj[0]['id_trunk_group3'] . ") AS CO LEFT OUTER JOIN pkg_call_online D ON CO.trunkcode = D.tronco WHERE D.tronco IS NULL ORDER BY starttime ASC";

        } else if ($this->tariffObj[0]['trunk_group_type'] == 3) {
            $sql1 = "SELECT *, (SELECT buyrate FROM pkg_rate_provider WHERE id_provider = tr.id_provider AND id_prefix = " . $this->tariffObj[0]['id_prefix'] . " LIMIT 1) AS buyrate  FROM pkg_trunk_group_trunk t  JOIN pkg_trunk tr ON t.id_trunk = tr.id WHERE id_trunk_group = " . $this->tariffObj[0]['id_trunk_group'] . " ORDER BY buyrate IS NULL , buyrate ";
            $sql2 = "SELECT *, (SELECT buyrate FROM pkg_rate_provider WHERE id_provider = tr.id_provider AND id_prefix = " . $this->tariffObj[0]['id_prefix'] . " LIMIT 1) AS buyrate  FROM pkg_trunk_group_trunk t  JOIN pkg_trunk tr ON t.id_trunk = tr.id WHERE id_trunk_group = " . $this->tariffObj[0]['id_trunk_group2'] . " ORDER BY buyrate IS NULL , buyrate ";
            $sql3 = "SELECT *, (SELECT buyrate FROM pkg_rate_provider WHERE id_provider = tr.id_provider AND id_prefix = " . $this->tariffObj[0]['id_prefix'] . " LIMIT 1) AS buyrate  FROM pkg_trunk_group_trunk t  JOIN pkg_trunk tr ON t.id_trunk = tr.id WHERE id_trunk_group = " . $this->tariffObj[0]['id_trunk_group3'] . " ORDER BY buyrate IS NULL , buyrate ";
        }
        ...
        $modelTrunks1 = $agi->query($sql1)->fetchAll(PDO::FETCH_OBJ);

        foreach ($modelTrunks1 as $key => $trunk) {
        ...
        $modelTrunks2 = $agi->query($sql2)->fetchAll(PDO::FETCH_OBJ);

        foreach ($modelTrunks2 as $key => $trunk) {
        ...
        $modelTrunks3 = $agi->query($sql3)->fetchAll(PDO::FETCH_OBJ);

        foreach ($modelTrunks3 as $key => $trunk) {
